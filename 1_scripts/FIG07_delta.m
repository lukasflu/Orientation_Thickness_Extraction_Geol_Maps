% | -----------------------------------------------------------------------
% |
% | ---- PLOT RELIABILITY INDICATOR DELTA (THICKNESS DIFFERENCE RATIO) ----
...
%% Figure 6, plot filtering parameter delta -------------------------------
...
% create two subsets with delta values classified reliable / unreliable
ind_delta_reliable = find(...
     outputtable4filtering(:,7)   <  thickness_diff_threshold & ...
     outputtable4filtering(:,13)   == 2);

ind_delta_unreliable = find(...
     outputtable4filtering(:,7)   >=  thickness_diff_threshold & ...
     outputtable4filtering(:,13)   == 2);
...
% PLOT UNRELIABLE DELTA VALUES FIRST --------------------------------------
% for loop plots only every 10th point to enhance visibility
for m = 1:10:length(ind_delta_unreliable)
    scatter(...
        outputtable_thickness(ind_delta_unreliable(m),1),...
        outputtable_thickness(ind_delta_unreliable(m),2),...
        symbol_size*0.4,outputtable_thickness(ind_delta_unreliable(m),7),...
        'o','LineWidth',0.5);
    hold on;
end

% PLOT RELIABLE DELTA VALUES ----------------------------------------------
% for loop plots only every 10th point to enhance visibility
for m = 1:10:length(ind_delta_reliable)
    scatter(...
        outputtable_thickness(ind_delta_reliable(m),1),...
        outputtable_thickness(ind_delta_reliable(m),2),...
        symbol_size*0.4,outputtable_thickness(ind_delta_reliable(m),7),...
...
c = colorbar; set( c, 'YDir'); caxis([0 1]);
colormap(parula);
c.Label.String = 'Relative Thickness difference []';
