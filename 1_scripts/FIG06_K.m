% | -----------------------------------------------------------------------
% |
% | --- PLOT RELIABILITY INDICATOR K (RELIABILITY OF ORIENTATION INFO) ----
...
% select reliable K-values
ind_K_reliable  = find(K_mat(:,1) <= K_value_threshold);
% select not reliable K-values
ind_K_unreliable  = find(K_mat(:,1) > K_value_threshold);

% create matrix including all XYZ_coordinates
XYZ_Coo = cell2mat(transpose(xyzOrientationData));

% PLOT UNRELIABLE K VALUES FIRST
% for loop plots only every 10th point to enhance visibility
for m = 1:10:length(ind_K_unreliable)
    scatter(...
        XYZ_Coo(ind_K_unreliable(m),1),...
        XYZ_Coo(ind_K_unreliable(m),2),...
        symbol_size*0.4,K_mat(ind_K_unreliable(m)),...
        'o','LineWidth',0.5);
    hold on;
end

% PLOT RELIABLE K VALUES
% for loop plots only every 10th point to enhance visibility
for m = 1:10:length(ind_K_reliable)
    scatter(...
        XYZ_Coo(ind_K_reliable(m),1),...
        XYZ_Coo(ind_K_reliable(m),2),...
        symbol_size*0.4,K_mat(ind_K_reliable(m)),...
...
% color axis for K-values, the smaller the better the fit
c = colorbar; set( c, 'YDir'); caxis([0 5])
colormap(parula);
c.Label.String = 'K-value (Fernandez, 2005)';
