% | -----------------------------------------------------------------------
% |
% | - PLOT RELIABILITY INDICATOR ALPHA (PARALLELISM OF THICKNESS VECTORS) -
...
%% Figure 8, plot filtering parameter alpha--------------------------------
...
for m = 1:10:length(ind_alpha_unreliable)
    scatter(...
        outputtable_thickness(ind_alpha_unreliable(m),1),...
        outputtable_thickness(ind_alpha_unreliable(m),2),...
        symbol_size*0.4,outputtable_thickness(ind_alpha_unreliable(m),8),...
        'o','LineWidth',0.5);
    hold on;
end
...
% for loop plots only every 10th point to enhance visibility
for m = 1:10:length(ind_alpha_reliable)
    scatter(...
        outputtable_thickness(ind_alpha_reliable(m),1),...
        outputtable_thickness(ind_alpha_reliable(m),2),...
        symbol_size*0.4,outputtable_thickness(ind_alpha_reliable(m),8),...
...
c = colorbar; set( c, 'YDir'); caxis([0 90]);
colormap(parula);
c.Label.String = 'Angular difference [°]';
