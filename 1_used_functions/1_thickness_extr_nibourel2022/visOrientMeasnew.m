...
for m = 1:length(ORattr)
    ...
    amp1    = (length(X)+length(Y))/(2*60); % modif. LN: sign size - how much to amplify vector (small dip vector)
    %amp2    = (length(X)+length(Y))/25;     % sign size - how much to amplify vector (large strike vector)
    amp2    = (length(X)+length(Y))/60;     % sign size - how much to amplify vector (large strike vector)
    maxz    = max(max(Z));                  % maximum Z value (set signs at the highest point so that no sign intersects with the topography)
    lwdt    = 1;                            % Line Width (default = 3, modification LN: lwdt = 1)
    ...
    ps2 = plot3([x, x+ xs2*amp2],[y, y + ys2*amp2],[maxz, maxz]); hold on
        ...
        ps2.Color    = [0,0,0,1];
    text(double(x - xd*amp1), double(y - yd*amp1), double(maxz+10), num2str(dip),'FontSize', 8);
    hold on
end