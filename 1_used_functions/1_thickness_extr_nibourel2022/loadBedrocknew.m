...
function [BEDcoor, BEDattr] = loadBedrocknew(bedshape,X,Y)
...
% ----------
% INPUT
% bedshape     --> name of shapefile containing bedrock data (polygon
%                   data)
...
bbox = [X(1)-1 Y(length(Y))-1; X(length(X))+1 Y(1)+1];      % added by LN: 2021-12-06, only load relevant parts of shapefile

[BEDcoor,BEDattr] = shaperead(bedshape,'BoundingBox',bbox);
